    ...
    for item in shopping_list:
        ...
        if found:
            if div not in user.current_divisions_list.keys():
                ...
                user.current_divisions_list[div] = (
                    user.current_divisions_list[div] + "\n" + item
                )
...
def process_add(message):
    ...
    if div not in user.json_data["מחלקות"]:
        bot.send_message(
            chat_id,
            "נראה שהמחלקה שהזנת לא מתאימה למחלקות הקיימות עליך לנסות שוב מהתחלה על ידי שליחת add\ ",
        )
        return
    for i in range(len(lines)):
        if i == 0:
            ...
            if i % 2 == 1:
                ...
                if found:
                    bot.send_message(
                        chat_id, "הפריט " + item + " כבר מופיע במחלקה " + str(div)
                    )
                else:
